...
#[tokio::test(flavor = "multi_thread")]
async fn e2e_go_mod_publishes_outdated_version_warning() {
    // 1. Setup real Cache with test data (oldest first, newest last)
    let (_temp_dir, cache) = create_test_cache(
...
#[tokio::test(flavor = "multi_thread")]
async fn e2e_go_mod_no_diagnostics_for_latest_version() {
    // 1. Setup real Cache with test data (oldest first, newest last)
    let (_temp_dir, cache) = create_test_cache(
        RegistryType::GoProxy,
...
#[tokio::test(flavor = "multi_thread")]
async fn e2e_go_mod_publishes_error_for_nonexistent_version() {
    // 1. Setup real Cache with test data (oldest first, newest last)
    let (_temp_dir, cache) = create_test_cache(
        RegistryType::GoProxy,
        &[("golang.org/x/text", vec!["v0.13.0", "v0.14.0"])],
...
#[tokio::test(flavor = "multi_thread")]
async fn e2e_go_mod_require_block_publishes_outdated_version_warning() {
    // 1. Setup real Cache with test data (oldest first, newest last)
    let (_temp_dir, cache) = create_test_cache(
        RegistryType::GoProxy,
        &[
